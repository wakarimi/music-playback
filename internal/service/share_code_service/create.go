...
func (s Service) Create(tx *sqlx.Tx, shareCode model.ShareCode, roomID int, accountID int) (err error) {
	log.Debug().Int("roomID", roomID).Msg("Creating share code")
	...
	err = s.ShareCodeRepo.Create(tx, shareCode, roomID)
	if err != nil {
